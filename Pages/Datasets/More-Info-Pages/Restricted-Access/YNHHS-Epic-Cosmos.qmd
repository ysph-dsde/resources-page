...
Researchers wanting to use the Yale New Haven Health System (YNHHS) Epic Cosmos data will require a Researcher Basic Access (RBA) account. Qualifying researchers will receive a YNHHS NetID and Duo Access, which affords access to:
...
::: {.column width="72%"}
Remote access to a YNHHS secure device without requiring the YNHHS VPN. VDI includes the following software: SQL Server Management Studio, R, Stata, SAS, Office365, and Epic. It can be used to handle small-to-medium sized data, accessing Epic charts, SlicerDicer, and the CHP.

Access de-identified summary count data representing the entire YNHHS patient dataset for data exploration, analytics, and reporting. IRB applications is not required, but training is strongly recommended before usage.
...
All Yale University and YNHHS (YU-YNHHS) Covered Entity faculty and licensed provides may submit and sponsor RBA requests. If you are within the Yale system, your manager or sponser with a YNHHS account must complete the ["YNHHS Researcher Basic Access"](https://shorturl.at/nJOS0) request on your behalf to start the process.
...
3.  Human Subjects Protection Training (HSPT) certificate from CITI completed within the last 3 years. Below is a summary of the process, and the complete directions can be found on the [It's Your Yale](https://your.yale.edu/research-support/human-research-protection-program/education-and-training/human-research-training) page about HSPT. You can reach out to irb.training\@yale.edu for further questions.
    ...
    For Yale-affiliates, it is preferable, though not required, to go through Workday to access the CITI training modules. Doing so will expedite recognition of the certificate in the Yale Integrated Research Enterprise Solution (IRES) Institutional Review Board (IRB) system. In Workday, the CITI training link will show up as the "Human Subjects Protection Training – Initial/Refresher" course. You can reach out to your manager, learning\@yale.edu, or irb.training\@yale.edu to request manual assignment if needed.
